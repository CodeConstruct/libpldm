...
#include <libpldm/libpldm-sizes.h>
...
#define debug_printf(fmt, ...) do { if (printf_enabled) fprintf(stderr, fmt, ##__VA_ARGS__);} while (0)
...
static const uint8_t FIXED_DESCRIPTORS[] = {
	// iana type
	0x01, 0x00,
	// length
	0x04, 0x00,
...
static uint8_t cb_device_identifiers(void* ctx LIBPLDM_CC_UNUSED,
    uint32_t *descriptors_len, uint8_t *descriptors_count,
    const uint8_t **descriptors)
{
	debug_printf("cb_device_identifiers\n");
	*descriptors_count = 1;
	*descriptors_len = sizeof(FIXED_DESCRIPTORS);
...
static uint8_t cb_imageset_versions(void* ctx,
...
static enum pldm_component_response_codes cb_update_component(void *ctx,
...
static uint8_t cb_firmware_data(void* ctx, uint32_t offset,
...
static uint8_t cb_verify(void* ctx, const struct pldm_firmware_update_component *comp,
    bool *ret_pending, uint8_t *ret_percent_complete LIBPLDM_CC_UNUSED)
{
...
static uint8_t cb_apply(void* ctx, const struct pldm_firmware_update_component *comp,
    bool *ret_pending, uint8_t *ret_percent_complete LIBPLDM_CC_UNUSED)
{
...
static void cb_cancel_update_component(void* ctx, const struct pldm_firmware_update_component *comp) 
{
...
extern "C"
int LLVMFuzzerInitialize(int *argc LIBPLDM_CC_UNUSED, char ***argv LIBPLDM_CC_UNUSED)
{
...
int LLVMFuzzerTestOneInput(uint8_t *input, size_t len)
{
	...
	ssize_t proto_size = len - FUZZCTRL_SIZE;
	rc = pldm_msgbuf_init_errno(fuzzctrl, 0, input, FUZZCTRL_SIZE);
	assert(rc == 0);
	rc = pldm_msgbuf_init_errno(fuzzproto, 0, &input[FUZZCTRL_SIZE], proto_size);
	assert(rc == 0);
	...
	auto fd_storage = std::make_unique<uint8_t[]>(PLDM_SIZEOF_PLDM_FD);
	struct pldm_fd *fd = reinterpret_cast<struct pldm_fd*>(fd_storage.get());

	rc = pldm_fd_setup(fd, PLDM_SIZEOF_PLDM_FD, NULL, &fuzz_ops, ops_ctx.get());
	assert(rc == 0);
	...
	while (1) {
		...
		if (fuzz_chance(ops_ctx.get(), PROGRESS_PERCENT)) {
			...
			rc = pldm_msgbuf_extract_array_uint8(fuzzproto, part_len, part_buf.data(), part_buf.size());
			if (rc != 0) {
				break;
			}
